...
export function StoreShell({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="min-h-[100dvh] bg-[#0A0A0A] text-[#F9FAFB] pb-24 md:pb-12" dir={dir}>
      ...
      {/* Floating Desktop Quick Dock */}
      <nav
        aria-label="Desktop Dock"
        className="hidden md:flex fixed bottom-6 left-1/2 -translate-x-1/2 z-40 items-center justify-between gap-8 rounded-full px-7 py-2.5 bg-[#141414]/90 backdrop-blur-xl border border-white/10 shadow-2xl transition-all"
      >
        <Link
          href="/"
          ...
          className={`flex items-center justify-center transition-all ${
            location === '/'
              ? 'size-10 rounded-full bg-[#D4A5A5] text-[#0A0A0A] shadow-md scale-105'
              : 'size-9 text-[#A1A1AA] hover:text-white hover:scale-105'
          }`}
        >
        ...
        >
          ...
          {count > 0 && (
            <span className="absolute -top-1 -right-1 flex size-4 items-center justify-center rounded-full bg-[#D4A5A5] text-[9px] font-bold text-[#0A0A0A]">
              {count}
            </span>
          )}
        </Link>
        ...
        <button
          ...
          aria-label={t('nav.wishlist')}
          onClick={() => setWishlistDrawerOpen(true)}
          className="relative flex size-9 items-center justify-center text-[#A1A1AA] hover:text-white hover:scale-105 transition-all"
        >
          <Heart
            className={`size-4.5 ${wishlist.length > 0 ? 'fill-[#D4A5A5] text-[#D4A5A5]' : ''}`}
            strokeWidth={1.5}
          />
          {wishlist.length > 0 && (
            <span className="absolute -top-1 -right-1 flex size-4 items-center justify-center rounded-full bg-[#D4A5A5] text-[9px] font-bold text-[#0A0A0A]">
